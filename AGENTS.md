...
- URIs must follow `/ <domain> /v<version> /collection-resource` and `/ <domain> /v<version> /collection-resource/{resource-id}`.
...
- PRs should include a short summary, relevant spec paths (e.g., `specs/core/api/v1/openapi.json`), and validation notes if run.

## Agent-Specific Instructions

- Keep private schemas within their API folder; move to `specs/common/` or `specs/<domain>/common/` when cross-API reuse is needed.
